"""Basic script to allow Graal Agent runs with multiple inputs."""
...
# Template for each execution block
execution_template = """
\t\t<execution>
\t\t\t<id>java-agent-{id}</id>
\t\t\t<goals>
...
\t\t\t\t\t<argument>-i</argument>
\t\t\t\t\t<argument>{input_message}</argument>
\t\t\t\t\t<argument>-o</argument>
...
input_messages = [msg.replace("&", "&amp;") for msg in input_messages]
input_messages = [msg.replace("<", "&lt;") for msg in input_messages]
input_messages = [msg.replace(">", "&gt;") for msg in input_messages]
input_messages = [msg.replace('"', "&quot;") for msg in input_messages]
input_messages = [msg.replace("'", "&apos;") for msg in input_messages]

# Create the full executions content by filling in the template for each message
executions_content = "".join(
    execution_template.format(id=index + 1, input_message=message)
    for index, message in enumerate(input_messages)
)

# Combine the base plugin content with the executions content
full_content = base_plugin.format(executions_content)
